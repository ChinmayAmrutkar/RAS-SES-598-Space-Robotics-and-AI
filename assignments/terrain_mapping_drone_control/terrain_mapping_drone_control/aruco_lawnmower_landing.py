...
class ArucoLawnmowerMission(Node):
    def __init__(self):
        ...
        self.landing_target = None  # Store the final landing position
    ...
    def image_callback(self, msg):
        # Skip if necessary data is not available or if ArUco is already detected
        ...
        if not self.lawnmower_started:  # Do not detect ArUco until survey starts
            return
        if self.landing_target is not None:  # We already have a landing target
            return
        ...
        try:
            ...
            if ids is not None and len(corners) > 0:
                ...
                y = self.vehicle_odometry.position[1] + dx - 01.0
                
                # Save the position only once - this will be our final landing target
                self.landing_target = (x, y)
                self.aruco_detected = True
                self.state = "GOTO_ARUCO"
                self.get_logger().info(f"--- ArUco detected. Setting landing target to: ({x:.2f}, {y:.2f})")
                
                # Unsubscribe from the image topic to stop further ArUco detection
                self.destroy_subscription(self.image_sub)
                self.get_logger().info("--- Stopped ArUco detection. Using fixed landing coordinates.")
    ...
    def timer_callback(self):
        ...
        if self.state == "WAITING_FOR_ARM":
            ...
            if self.landing_target:  # If we have a landing target, go to it
                self.state = "GOTO_ARUCO"
                return
            ...
            if self.landing_target:
                ...
                if math.hypot(x - self.vehicle_odometry.position[0], y - self.vehicle_odometry.position[1]) < 0.3:
                    ...
                    if self.aruco_stable_counter > 10:
                        self.state = "LAND"
                        self.get_logger().info("--- Position reached. Starting landing...")
                else:
                    self.aruco_stable_counter = 0
            ...
            if self.landing_target:
                x, y = self.landing_target
                # Keep the exact X,Y position during descent
